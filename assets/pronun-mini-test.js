...
async function ttsPlay(text, voice="shimmer", speed=1.0){
  ...
  // Attach simple playback time measurement (handles play/pause/resume)
  (function attachPlaybackTimer(audioEl){
    let playStart = null; let accumulatedMs = 0;
    audioEl.addEventListener('playing', ()=>{ if (playStart === null) playStart = (performance && performance.now) ? performance.now() : Date.now(); });
    audioEl.addEventListener('pause', ()=>{ if (playStart !== null){ const now = (performance && performance.now) ? performance.now() : Date.now(); accumulatedMs += now - playStart; playStart = null; } });
    audioEl.addEventListener('ended', ()=>{ if (playStart !== null){ const now = (performance && performance.now) ? performance.now() : Date.now(); accumulatedMs += now - playStart; playStart = null; } try{ audioEl.measuredPlaySec = Number((accumulatedMs/1000).toFixed(2)); }catch(_){ audioEl.measuredPlaySec = null; } });
    // expose a helper to get the best-estimated duration in seconds
    audioEl.getBestDurationSec = function(){
      // priority: explicit durationEstimateSec (server or decoded), HTMLMediaElement.duration, measuredPlaySec
      try{
        if (typeof this.durationEstimateSec === 'number' && !Number.isNaN(this.durationEstimateSec)) return this.durationEstimateSec;
        if (typeof this.duration === 'number' && isFinite(this.duration) && this.duration > 0) return Number(this.duration);
        if (typeof this.measuredPlaySec === 'number') return this.measuredPlaySec;
      }catch(_){ }
      return null;
    };
  })(a);
  ...
  // prefer server-provided estimate, but attempt to decode the audio blob
  // to obtain an exact duration via Web Audio API when available.
  a.durationEstimateSec = (typeof data.durationEstimateSec === 'number') ? data.durationEstimateSec : null;
  try{
    // decode for precise duration (some browsers provide reliable .duration after loadedmetadata,
    // but decoding gives a dependable value for the audio content)
    const maybeDecode = async () => {
      try{
        const ab = await blob.arrayBuffer();
        const AudioCtx = window.AudioContext || window.webkitAudioContext;
        if (!AudioCtx) return;
        const ctx = new AudioCtx();
        // decodeAudioData may accept a promise-returned buffer in modern APIs
        const decoded = await new Promise((resolve, reject) => {
          try{
            ctx.decodeAudioData(ab.slice(0), resolve, err=>reject(err));
          }catch(e){ reject(e); }
        });
        if (decoded && typeof decoded.duration === 'number'){
          // store decoded duration (seconds) as preferred estimate
          a.durationEstimateSec = Number(decoded.duration.toFixed(2));
          a.decodedDurationSec = a.durationEstimateSec;
        }
...
<button class="badge duration-badge" data-durations title="발음 속도 / Temps de prononciation">
  ...
  const accuracyBadge = wrap.querySelector('.accuracy-badge');
  const durationBadge = wrap.querySelector('.duration-badge');
            if (accuracyBadge) {
  ...
  // Normalize recorded duration: Pronun widget may return milliseconds or seconds.
  // If value looks like milliseconds (>> 10), convert to seconds.
  let myRec = null;
  try {
    if (typeof duration === 'number' && !Number.isNaN(duration)) {
      // If duration is unusually large, assume milliseconds and convert.
      if (duration > 10) {
        // likely milliseconds
        myRec = Number((duration / 1000).toFixed(2));
        // helpful debug for developers (can be removed later)
        if (typeof console !== 'undefined' && console.warn) console.warn('[pronun-mini-test] converted recorded duration from ms to s:', duration, '->', myRec);
      } else {
        myRec = Number(duration.toFixed(2));
      }
    }
  } catch (e) { myRec = null; }
  const ttsNum = ttsDur ? Number(ttsDur.toFixed(2)) : null;
        ...
        if (durationBadge) {
          const t = Number(ttsNum || 0); const r = Number(myRec || 0);
            // Estimate average duration for this reference sentence using
            // stored seconds-per-syllable rates. Compute syllable count for
            // the reference sentence and pass it to the average helper.
            const refSyllables = (sent.ko || '').split('').filter(ch => /[\uAC00-\uD7A3]/.test(ch)).length || 0;
            const avgLocal = computeLocalAverageSeconds(refSyllables);
    ...
    // store normalized seconds in samples and rate (seconds per syllable)
    const sampleDuration = (typeof myRec === 'number' ? myRec : null);
    const sampleRate = (sampleDuration !== null && syCount && syCount > 0) ? Number((sampleDuration / syCount).toFixed(4)) : null;
    const sample = { duration: sampleDuration, syllables: syCount, rate: sampleRate };
      ...
      // update len-compare area
        try{
          ...
          if (lenWrap){
            const t = Number(ttsDur || 0); const r = Number(myRec || 0);
            // update badges
// update badges (DO NOT overwrite innerHTML which contains the bar visuals)
if (durationBadge) {
  const txt = `평균 속도 / Vitesse moyenne: ${ttsDur ? ttsDur.toFixed(1) + 's' : '?s'} · 내 속도 / Ma vitesse: ${myRec ? myRec.toFixed(1) + 's' : '?s'}`;
  try {
  ...
  try {
    ...
    compact.textContent = txt;
            ...
            if (rBar){
              ...
              rBar.title = `녹음: ${myRec?myRec.toFixed(2)+'s':'?s'}`;
            }
          }
        }catch(_){ }
            ...
            if (!wrap.querySelector('.hyp-reval')){
              ...
              rbtn.addEventListener('click', ()=>{
                ...
                try{
                  ...
                  scoreBox.textContent = `${pct}% · 길이: ${myRec?.toFixed?.(1)||'?'}s`;
        ...
        try{
          const cardEl = wrap;
            if (cardEl) {
            // persist normalized durations (seconds)
            if (typeof myRec === 'number') cardEl.dataset.recDuration = String(Number(myRec.toFixed(2)));
  ...
  // also update compact score box (use normalized recorded duration)
  scoreBox.textContent = `${pct}% · 길이: ${myRec?.toFixed?.(1)||'?'}s`;
      } catch (e) {
  ...
  hintWrap.innerHTML = `<div class="hint-wrap"><button class="btn btn-ghost btn-sm hint-help1" data-hint="1">도와주세요 / Aidez‑moi</button> <button class="btn btn-ghost btn-sm hint-help2" data-hint="2">살려주세요 / Sauvez‑moi</button> <span class="hint-display" data-hint-display></span></div>`;
  ...
  // hintDisplay element will be declared once below

    // 도움받기1: 초성 + 주요단어 표시 (초성은 직접 생성, 주요단어는 sent.hint1이 제공되면 사용)
    const help1Btn = hintWrap.querySelector('[data-hint="1"]');
...
function computeLocalAverageSeconds(refSyllables){
  try{
    const key = 'pronunSpeedSamples';
    const samples = JSON.parse(localStorage.getItem(key) || '[]') || [];
    if (!samples || !samples.length) return null;
    // If caller provided refSyllables, compute mean seconds-per-syllable
    // from stored rates and return estimated seconds for the reference.
    const rates = samples.map(s=>s && s.rate ? Number(s.rate) : null).filter(r=>typeof r==='number' && isFinite(r));
    if (typeof refSyllables === 'number' && refSyllables > 0 && rates.length){
      const meanRate = rates.reduce((a,b)=>a+b,0) / rates.length; // seconds per syllable
      return Number((meanRate * refSyllables).toFixed(2));
    }
    // Fallback: compute simple mean of raw durations (seconds)
    const durations = samples.map(s=>s && Number(s.duration)).filter(d=>typeof d==='number' && isFinite(d) && d>0);
