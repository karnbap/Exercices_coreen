...
// 틀린 발음 부분 하이라이트 함수 추가
function highlightErrors(reference, userPronunciation) {
  const refWords = reference.split(/\s+/);
  const userWords = userPronunciation.split(/\s+/);

  return userWords.map((word, index) => {
    if (word !== refWords[index]) {
      return `<span class='pronun-error'>${word}</span>`;
    }
    return word;
  }).join(' ');
}
...
// Example usage:
// const refText = "밖에 비가 쏟아져서 우산 좀 빌려 줄래?";
// const userText = "밖에 비가 쏟아져서 우산 좀 빌려 줄래.";
// const highlighted = highlightErrors(refText, userText);
// document.querySelector('[data-diff]').innerHTML = highlighted;
