// Warm-up 5×5 — 듣기 강조 + Turbo(2.0×) 발음 보정
...
const state = {
  mode: { speed:1.0, continuous:false }, // turbo는 continuous=false
  progress: {}, listenCount: {}, startISO:null, startMs:0, name:'Élève'
};
...
// (구) 연음 규칙은 Turbo에서 끄고, 일반속도만 보수적으로 적용
function applySimpleLiaisonForNumbers(text, useLiaison){
  if(!useLiaison) return text;
  const t = splitTokens(text); const out=[];
  for(let i=0;i<t.length;i++){
    if(t[i]==='일' && t[i+1]==='이'){ out.push('이리'); i++; }
    else out.push(t[i]);
  }
  return out.join(' ');
}

function makeTTSContinuous(text, speed=1.0, continuous=true){
  const provider = (window.PONGDANG_TTS?.provider) || 'openai';
  const useLiaison = continuous && speed < 1.6;               // 1.5×까지만 연음
  const raw = applySimpleLiaisonForNumbers(text, useLiaison);
  const parts = splitTokens(raw);

  // Turbo(≥1.9): 단어 사이 아주 짧은 쉬기(10ms)로 뭉개짐 방지
  if(provider === 'google'){
    const rate = Math.round(speed*100)+'%';
    const join = (speed >= 1.9) ? '<break time="10ms"/>' : '<break time="0ms"/>';
    return { ssml: `<speak><prosody rate="${rate}">${parts.join(join)}</prosody></speak>` };
  }
  if(speed >= 1.9){
    return { text: parts.join(' ') }; // 워드조이너 금지: Turbo 정확도 우선
  }
  return { text: parts.join('\u2060') }; // WORD JOINER (연속 읽기)
}

function mapVoice(provider, req){
  const MAP = {
    openai: { default:'alloy', alloy:'alloy', shimmer:'verse', verse:'verse' },
...
async function playTTS(input, voice='alloy', speed=1.0, btn, onStart){
  ...
  if(audioLock){
    if(currentAudio){ if(currentAudio.paused){ await currentAudio.play(); setBtnPlaying(btn,true); } else { currentAudio.pause(); setBtnPlaying(btn,false); } }
    return;
  }
  ...
  try{
    ...
    let data;
    try{ data = await fetchAudioPayload(payload, aborter.signal); }
    catch(e){ await new Promise(r=>setTimeout(r,800)); data = await fetchAudioPayload(payload, aborter.signal); }
  ...
  }catch(e){ showToast('⚠️ Audio: réseau lent. Réessaie / 네트워크 지연'); }
}
...
// -------- Recorder (동일)
function makeRecorder(){ let mediaRecorder=null, chunks=[], stream=null, ctx=null, analyser=null, raf=0;
  async function start(canvas){ if(stream) stop(canvas);
    ...
    source.connect(analyser); drawVU(canvas, analyser); mediaRecorder.start(50);
  }
  function drawVU(canvas, analyser){ if(!canvas) return;
    const cv = canvas, g = cv.getContext('2d'); const data = new Uint8Array(analyser.frequencyBinCount);
    const w = cv.width, h = cv.height; function loop(){ raf=requestAnimationFrame(loop);
      analyser.getByteFrequencyData(data); g.clearRect(0,0,w,h); const bars=32, step=Math.floor(data.length/bars);
      g.fillStyle='#6366f1'; for(let i=0;i<bars;i++){ const v=data[i*step]/255, bh=v*h; g.fillRect(i*(w/bars)+2, h-bh, (w/bars)-4, bh); } }
    loop();
  }
  function stop(canvas){ if(mediaRecorder && mediaRecorder.state==='recording') mediaRecorder.stop();
    if(stream){ stream.getTracks().forEach(t=>t.stop()); stream=null; } if(ctx){ ctx.close(); ctx=null; }
    if(raf) cancelAnimationFrame(raf); if(canvas){ const g=canvas.getContext('2d'); g.clearRect(0,0,canvas.width,canvas.height); }
  }
  async function getResult(){ return await new Promise((resolve)=>{
    const onStop=()=>{ const blob=new Blob(chunks,{type:'audio/webm'}); const reader=new FileReader(); const a=document.createElement('audio');
      a.src=URL.createObjectURL(blob); a.addEventListener('loadedmetadata', ()=>{ const duration=a.duration; reader.onloadend=()=>resolve({base64:reader.result,duration,blob}); reader.readAsDataURL(blob); });
    };
    if(mediaRecorder && mediaRecorder.state==='recording'){ mediaRecorder.addEventListener('stop', onStop, {once:true}); mediaRecorder.stop(); } else onStop();
  });}
...
async function analyzePronunciation({ referenceText, record }){
  const payload = { referenceText, audio:{ base64: toBareBase64(record.base64), filename:`rec_${Date.now()}.webm`, mimeType:'audio/webm', duration: record.duration } };
  ...
  const data = await r.json().catch(()=> ({})); if(!r.ok) throw new Error(data?.error || 'Analyse échouée');
  let acc = (typeof data.accuracy === 'number') ? data.accuracy : 0; if(acc>1) acc=acc/100;
...
function renderFeedback(box, {friendly=[], accuracy=0, transcript='', refText=''}) {
  if(!box) return;
  const hasSTT = !!transcript; const percent = hasSTT ? Math.round((accuracy||0)*100) : 0;
  const lines = [];
  if (!hasSTT) lines.push('🎧 Reconnaissance instable — réessaie dans 2–3 sec. / 음성 인식이 불안정해요. 2–3초 후 다시 시도하세요.');
  else if (percent === 100) lines.push('✅ Parfait ! Garde le rythme. / 완벽해요! 지금 리듬 유지.');
  if (Array.isArray(friendly)) friendly.forEach(it=>{ if(typeof it==='string') lines.push(it); else if(it && (it.tip||it.message||it.tag)) lines.push(String(it.tip||it.message||it.tag)); });
  try{
    const extra1 = (window.PronunUtils?.quickTips?.(refText, transcript)) || [];
    const extra2 = (window.VowelMiddleware?.hints?.(refText, transcript)) || [];
    [...extra1, ...extra2].forEach(t => { if(typeof t==='string') lines.push(t); else if(t && (t.tip||t.message)) lines.push(String(t.tip||t.message)); });
  }catch(_){}
...
function renderAll(){
  ...
  BUNDLES.forEach(b=>{ state.progress[b.key] = { done:false, score:null, accuracy:null, audioBase64:null, duration:null, friendly:[] }; state.listenCount[b.key]=0; wrap.appendChild(makeBundleCard(b)); });
  updateProgress(0); document.getElementById('finish-wrap')?.classList.add('hidden');
}

function makeBundleCard(bundle){
  const wrap = document.createElement('div'); wrap.className = 'p-4 bg-white rounded-lg border';

  const continuous = !!state.mode.continuous;
  const refDisplay = continuous ? splitTokens(bundle.text).join(' ') : bundle.text;
  const refEval    = collapseKorean(bundle.text);
  const ttsInput   = makeTTSContinuous(bundle.text, state.mode.speed, continuous);

  wrap.innerHTML = `
    <div class="flex items-center justify-between gap-2 flex-wrap">
      <div>
        <div class="text-sm text-slate-500">Vitesse ${state.mode.speed}× ${continuous?'<span class="text-slate-500 text-xs">(continu)</span>':''}</div>
      ...
      <div class="flex items-center gap-2">
        <button class="btn btn-primary btn-sound-big btn-play"><i class="fas fa-play"></i> Écouter / 듣기</button>
  ...
  btnStart.addEventListener('click', async ()=>{
    ...
    catch(e){ showToast('🎙️ Autorise le micro / 마이크 권한 허용'); btnStart.disabled=false; btnStop.disabled=true; }
  });
  ...
  btnStop.addEventListener('click', async ()=>{
    ...
    try{
      ...
      status.textContent = lastRecord ? `Terminé ${(lastRecord.duration||0).toFixed(1)}s → “Évaluer” / 완료 → “평가”` : 'Réessaie / 다시 시도';
    }catch(e){ btnStart.disabled = false; status.textContent = 'Problème d’enregistrement. Réessaie. / 녹음 오류. 다시 시도.'; }
  });
  ...
  btnEval.addEventListener('click', async ()=>{
    ...
    try{
      ...
      const hasSTT = !!transcript; const percent = hasSTT ? Math.round((accuracy || 0)*100) : 0;
      scoreTag.textContent = `Score / 점수: ${percent}%`; scoreTag.classList.remove('hidden');
      state.progress[bundle.key] = { done:true, score:percent, accuracy, audioBase64: toBareBase64(lastRecord.base64), duration:lastRecord.duration, friendly };
      ...
      checkFinish();
    }catch(e){ status.textContent = 'Échec de l’évaluation. Réessaie. / 평가 실패. 다시 시도.'; }
    finally{ btnEval.disabled = false; }
  });
...
async function sendResults(){
  const questions = BUNDLES.map(b=>{
    const st = state.progress[b.key] || {}; const refText = collapseKorean(b.text);
    return {
      number:`WU-${b.key}`, type:'warmup_pronun',
      fr:`${b.label} — vitesse ${state.mode.speed}×${state.mode.continuous?' (continu)':''}`,
      ko:refText, userAnswer:'', isCorrect:true,
      listenCount: state.listenCount[b.key]||0, hint1Count:0, hint2Count:0,
  ...
  const payload = {
    ...
    assignmentSummary: ['4 groupes: Natifs(1–5,6–10) + Hanja(1–5,6–10)','Écouter → S’enregistrer → Évaluer (score en %)'],
    questions
  };

  try{
    const r = await fetch(`${FN_BASE}/send-results`, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload) });
    const j = await r.json().catch(()=> ({})); if(!r.ok || j?.ok===false) throw new Error(j?.error || 'send-results failed');
    showToast('📨 Résultat envoyé / 결과 전송 완료');
  }catch(e){ console.error(e); showToast('❌ Envoi impossible. Réessaie / 전송 실패'); }
}
...
function WU_go(mode){
  ...
  else if(mode==='turbo')state.mode = { speed:2.0, continuous:false }; // Turbo: 연속/연음 OFF
  ...
  renderAll(); const t = document.getElementById('warmup-screen'); if(t) window.scrollTo({ top:t.offsetTop-8, behavior:'smooth' });
}
window.WU_go = WU_go;

// -------- Mount
document.addEventListener('DOMContentLoaded', ()=>{ try{ window.StudentGate?.init?.(); window.StudentGate?.requireBeforeInteraction?.(document); }catch(_){ } });
